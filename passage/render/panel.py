...
def draw(surface: pygame.Surface, flow: Flow, cell: Cell, paused: bool,
    ...
    # What was dealt, then what was chosen. They sit together because they are
    # the two halves of the same question: this genome, on this food.
    if vigour is not None:
        typo.caps(surface, "eating", (x, 78), 9, palette.INK_FAINT, 1.6)
        typo.draw(surface, vigour.served, (x, 92), 11, palette.INK, 0.2)
        typo.draw(surface, f"{sum(vigour.eaten.values()):.0f} eaten",
                  (right, 92), 10, palette.PENCIL, 0.0, align="right")

    typo.caps(surface, "target", (x, 114), 9, palette.INK_FAINT, 1.6)
    built = lineage.biomass() if lineage is not None else cell.pool("biomass")
    typo.draw(surface, "biomass", (x, 130), 13, palette.INK, 0.3)
    typo.draw(surface, f"{built:.0f}", (right, 130), 13, palette.INK, 0.0,
              align="right")
...
def _carriers(surface: pygame.Surface, cell: Cell, x: float,
    ...
    """
    typo.caps(surface, "carriers", (x, 162), 9, palette.INK_FAINT, 1.6)
